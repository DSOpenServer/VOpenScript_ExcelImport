...
class SpreadsheetReader_XLSX implements Iterator, Countable
{
     ...
     * If null, shared strings are cached regardless of amount.
     */
    ...
    private static array $RuntimeInfo = [
        'GMPSupported' => false,
    ];
    ...
    // Worksheet file
    private string|false $WorksheetPath = false;
    private XMLReader|false $Worksheet  = false;

    // Shared strings file
    private string|false $SharedStringsPath = false;
    private XMLReader|false $SharedStrings   = false;
    private array $SharedStringCache         = [];

    // Workbook data
    private SimpleXMLElement|false $WorkbookXML = false;

    // Style data
    private SimpleXMLElement|false $StylesXML = false;
    private array $Styles                      = [];

    private string $TempDir  = '';
    private array $TempFiles = [];

    private array|false $CurrentRow = false;
    ...
    private static array $BuiltinFormats = [
        0  => '',
        1  => '0',
        2  => '0.00',
        3  => '#,##0',
    ...
    private static array $DateReplacements = [
        'All' => [
            '\\'   => '',
            'am/pm' => 'A',
            'yyyy' => 'Y',
    ...
    private static DateTime|false $BaseDate        = false;
    private static string $DecimalSeparator        = '.';
    private static string $ThousandSeparator       = '';
    private static string $CurrencyCode            = '';

    private array $ParsedFormatCache = [];

    /**
     * @param string     $Filepath Path to file
     * @param array|null $Options  Options:
     *                             TempDir               => string Temporary directory path
     *                             ReturnDateTimeObjects => bool   True => dates/times as DateTime objects
     *
     * @throws Exception
     */
    ...
    public function __destruct()
    {
        ...
        // Better safe than sorry — shouldn't try deleting '.', '/', or '..'.
    ...
    /**
     * Creates shared string cache if the number of shared strings is acceptably low
     * (or there is no limit on the amount).
     */
    private function PrepareSharedStringCache(): bool
    {
        while ($this->SharedStrings->read()) {
            if ($this->SharedStrings->name === 'sst') {
                $this->SharedStringCount = (int)$this->SharedStrings->getAttribute('count');
                break;
            }
        }
        ...
        if (
            ...
            (self::SHARED_STRING_CACHE_LIMIT !== null && self::SHARED_STRING_CACHE_LIMIT < $this->SharedStringCount)
        ) {
            return false;
        }
     ...
     *
     * @param int $Index Shared string index
     *
     * @return string Value
     */
    private function GetSharedString(int $Index): string
    {
        ...
        // If index of the desired string is larger than possible, don't bother.
    ...
    /**
     * Formats the value according to the style index.
     *
     ...
     */
    private function FormatValue(mixed $Value, int $Index): mixed
    {
        if (!is_numeric($Value)) {
            return $Value;
        }
        ...
        // Special case for "General" format
        if ($Index === 0) {
        ...
        if (!$Format) {
            ...
            // Stripping colors
            $Format['Code'] = trim(preg_replace('{^\[[[:alpha:]]+\]}i', '', (string)$Format['Code']));
            ...
            if (substr($Format['Code'], -1) === '%') {
                ...
                // Removing string quotes and asterisks
                ...
                if (preg_match('{#?.*\?\/\?}', $Format['Code'])) {
                    ...
                    if (preg_match('{(0+)(\.?)(0*)}', preg_replace('{\[[^\]]+\]}', '', $Format['Code']), $Matches)) {
                        ...
                        $Format['Precision']  = strlen($Decimals);
                ...
                if (preg_match('{\[\$(.*)\]}u', $Format['Code'], $Matches)) {
                    $CurrCode = explode('-', $Matches[1]);
                    $CurrCode = $CurrCode[0] ?: self::$CurrencyCode;
        ...
        if ($Format) {
            if ($Format['Code'] === '@') {
                ...
                if ($Format['Type'] === 'Fraction' && ($Value != (int)$Value)) {
                    ...
                    $Decimal       *= pow(10, strlen((string)$Decimal) - 2);
                    $DecimalDivisor = pow(10, strlen((string)$Decimal));

                    if (self::$RuntimeInfo['GMPSupported']) {
                        $GCD = gmp_strval(gmp_gcd((int)$Decimal, (int)$DecimalDivisor));
                    } else {
                        $GCD = self::GCD((int)$Decimal, (int)$DecimalDivisor);
                    }
     ...
     * @return mixed
     */
    public function GeneralFormat(mixed $Value): mixed
    {
        if (is_numeric($Value)) {
            $Value = (float)$Value;
        }
     ...
     *
     * @return array|false Current element from the collection
     */
            ...
            while ($this->Valid = $this->Worksheet->read()) {
                if ($this->Worksheet->name === 'row') {
                    ...
                    if ($RowSpans) {
                        ...
                        $CurrentRowColumnCount   = (int)$RowSpans[1];
                    } else {
            ...
            while ($this->Valid = $this->Worksheet->read()) {
                switch ($this->Worksheet->name) {
                    ...
                    case 'c':
                        ...
                        $CellRef = $this->Worksheet->getAttribute('r');
                        $Letter  = preg_replace('{[^[:alpha:]]}S', '', $CellRef);
     ...
     *
     * @return int|false Numeric index (0-based) or false if it cannot be calculated
     */
    public static function IndexFromColumnLetter(string $Letter): int|false
    {
    ...
    /**
     * Helper function for greatest common divisor calculation
     * in case the GMP extension is not enabled.
     *
